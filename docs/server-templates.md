...
**또는 자연어로**: `지우야` 라고 부르면서 같은 템플릿 파일을 **첨부**하고 "이대로 세팅해줘" 라고 하면
...
**템플릿 전체 적용** — `지우야` 라고 부르면서 템플릿 파일(`.yaml`/`.yml`/`.json`)을 **첨부**하고
셋업을 요청하세요. `/setup-server` 와 똑같이 미리보기 + `확인 실행` 버튼이 뜹니다.

```
지우야 이 파일대로 서버 세팅해줘   (+ server-template.yaml 첨부)
```
